...
public class CoreCodeGenController extends BaseController {
    ...
    @ResponseBody
    public Ret project(String path, String basePackage, String includeConsole) throws IOException {
        ...
        String tempalteRoot = "genTemplate/maven/";
    ...
    @PostMapping("/table.json")
    @ResponseBody
    public PageRet<Entity> getTable() {
        List<Entity> list = codeGenService.getAllEntityInfo();
        return PageRet.ok(list);
    }

    @PostMapping("/tableDetail.json")
    @ResponseBody
    public Ret<Entity> getInfo(String table) {
        Entity entity = codeGenService.getEntityInfo(table);
        if (entity == null) {
            Ret.fail("表不存在");
        }
    ...
    @PostMapping("/gen.json")
    @ResponseBody
    public Ret gen(EntityInfo data, String path) {
...
//				this.codeGenService.insertMenu(functionId, entity, urlBase);
    ...
    @ResponseBody
    public Ret<Map<String, String>> html(EntityInfo data) {
        Entity entity = getEntitiyInfo(data);
        String urlBase = data.getUrlBase();
        String basePackage = data.getBasePackage();
    ...
    @ResponseBody
    public Ret<Map<String, String>> javaCode(EntityInfo data) {
    ...
    @PostMapping("/sql.json")
    @ResponseBody
    public Ret<Map<String, String>> sql(EntityInfo data) {
    ...
    private Entity getEntitiyInfo(EntityInfo data) {
        ...
        if (StrUtil.isEmpty(entity.getCode()) || StrUtil.isEmpty(entity.getSystem())) {
